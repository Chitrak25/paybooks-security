				...
				if(accessService.existUserByEmailId(forgetPasswordRequest.getEmail())) {
					...
					if(userByMobile.getEmailId().equals(userByEmail.getEmailId())) {
						...
						User user = new User (userByEmail.getId(),userByEmail.getName(),userByEmail.getEmailId(),
								userByEmail.getGender(),userByEmail.getMobile(),encoder.encode(otp));
						...
						email = new EmailRequestDto("Paybooks Reset Password",userByEmail.getEmailId(),
								"username : " + userByEmail.getEmailId() + "   password : " + otp);
						...
						throw new Exception("User isn't present");
					...
					throw new Exception ("This E-mail adress doesn't exist.");
				}
					
		}
		else {
			throw new Exception("This mobile number doesn't exist");
		}
